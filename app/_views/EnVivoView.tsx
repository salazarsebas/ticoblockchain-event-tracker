...
type EnVivoViewProps = {
  now: Date;
  // ISO string when QA is time-travelling via /dev?now=… ; null on the
  // public route where the page is statically ISR-cached.
  simulated: string | null;
};

// Shared body for the En Vivo experience. The public `/` route renders this
// with `now = new Date()` and `simulated = null` so the page is fully static
// (ISR every 10s) and feels instant on prefetch. The `/dev` route renders
// this same view with a simulated time from `?now=` so QA can time-travel.
export default function EnVivoView({ now, simulated }: EnVivoViewProps) {
